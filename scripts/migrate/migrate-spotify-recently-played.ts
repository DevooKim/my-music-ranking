/**
 * spotify-recently-played 데이터 마이그레이션 스크립트
 *
 * 기능:
 * 1. spotify-recently-played/* 약 1800개 파일 처리
 * 2. Raw 데이터 마이그레이션 (주차별 병합 + 중복 제거 + 한국어 메타데이터 변환)
 * 3. Weekly 차트 재생성 (Raw 데이터 기반)
 * 4. Monthly 차트 재생성 (Raw 데이터 기반)
 * 5. track-stats 재생성
 */
...
// Phase 1: spotify-recently-played Raw 데이터 마이그레이션
async function migrateRawData(): Promise<Map<string, RawPlayedData>> {
  console.log(
    "📦 Phase 1: spotify-recently-played Raw 데이터 마이그레이션 시작...",
  );

  // 처리할 연월 범위 설정 (2025년 9월 ~ 현재)
  ...
  for (const yearMonth of yearMonths) {
    // 해당 연월의 파일만 조회 (S3 prefix 사용)
    const prefix = `spotify-recently-played/${yearMonth}`;
    const spotifyKeys = await listAllKeys(prefix);
    ...
    totalFiles += spotifyKeys.length;
    console.log(`\n  📅 ${yearMonth} 처리 시작 (${spotifyKeys.length}개 파일)...`);
    
    const BATCH_SIZE = 100; // 100개씩 배치 처리
    let processedFiles = 0;

    // 배치 단위로 처리
    for (let i = 0; i < spotifyKeys.length; i += BATCH_SIZE) {
      const batch = spotifyKeys.slice(i, i + BATCH_SIZE);
        ...
        try {
          ...
          for (const legacyItem of legacyData.items) {
            try {
              ...
              console.error(`      - item 변환 오류 (${key}):`, itemError);
            }
          }
          ...
          console.error(`      - 파일 처리 오류: ${key}`, error);
          processedFiles++;
          totalProcessedFiles++;
        }
      }
      ...
      console.log(
        `    - ${yearMonth} 진행: ${processedFiles}/${spotifyKeys.length} 파일 (전체: ${totalProcessedFiles}/${totalFiles}, ${weeklyData.size}개 주차)`,
      );
    }
  ...
  if (isIsrcEnrichmentEnabled()) {
    ...
    for (const [, weekMap] of weeklyData.entries()) {
      ...
      for (let i = 0; i < items.length; i++) {
        const enrichedItem = await enrichWithKoreanMetadata(items[i]);
        weekMap.set(getDeduplicationKey(enrichedItem), enrichedItem);
        enrichedCount++;

        if (enrichedCount % 100 === 0) {
          console.log(`    - 진행 중: ${enrichedCount}/${totalUnique} 트랙 처리 완료`);
        }
      }
    }
...
// 메인 실행
async function main() {
  console.log("🚀 spotify-recently-played 마이그레이션 시작\n");
  console.log(
